<template>
  <main class="relative h-full w-full transition-width flex flex-col overflow-hidden items-stretch flex-1">
    <div class="flex-1 overflow-hidden">
      <div class="react-scroll-to-bottom--css-walter-79elbk h-full dark:bg-gray-800">
        <div class="react-scroll-to-bottom--css-walter-1n7m0yu">
          <div v-if="conversation.length > 0" class="flex flex-col items-center text-sm dark:bg-gray-800">
            <div v-for="message in conversation"
              class="group w-full text-gray-800 dark:text-gray-100 border-b border-black/10 dark:border-gray-900/50"
              :class="{
                'dark:bg-gray-800': isClientMessage(message.source),
                'bg-gray-50 dark:bg-[#444654]': isServerMessage(message.source)
              }">
                  ...
                  <div class="flex flex-grow flex-col gap-3">
                    <div class="min-h-[20px] flex flex-col items-start gap-4 whitespace-pre-wrap">
                      <div class="markdown prose w-full break-words dark:prose-invert dark">
                        <p v-text="message.content.trim()"></p>
                      </div>
                    </div>
                  </div>
                  <div v-if="isServerMessage(message.source)" class="flex justify-between lg:block">
                    <div class="text-xs flex items-center justify-center gap-1 self-center pt-2 !invisible">
                      <button disabled="" class="dark:text-white disabled:text-gray-300 dark:disabled:text-gray-400">
                        ...
                        </svg></button><span class="flex-grow flex-shrink-0">1 / 1</span><button disabled=""
              ...
              <div class="md:flex items-start text-center gap-3.5">
                <div class="flex flex-col mb-8 md:mb-auto gap-3.5 flex-1">
                  <h2 class="flex gap-3 items-center m-auto text-lg font-normal md:flex-col md:gap-2">
                    <IconSun></IconSun>Examples
                  </h2>
                  <ul class="flex flex-col gap-3.5 w-full sm:max-w-md m-auto">
                    <button @click="setPrompt(item)" v-for="item in titleSections.examples.prompts"
                ...
                <div class="flex flex-col mb-8 md:mb-auto gap-3.5 flex-1">
                  <h2 class="flex gap-3 items-center m-auto text-lg font-normal md:flex-col md:gap-2">
                    <IconRay></IconRay>{{ titleSections.capabilities.title }}
                  </h2>
                  <ul class="flex flex-col gap-3.5 w-full sm:max-w-md m-auto">
                    <li v-for="item in titleSections.capabilities.prompts"
                ...
                <div class="flex flex-col mb-8 md:mb-auto gap-3.5 flex-1">
                  <h2 class="flex gap-3 items-center m-auto text-lg font-normal md:flex-col md:gap-2">
                    <IconDanger></IconDanger>{{ titleSections.limitations.title }}
                  </h2>
                  <ul class="flex flex-col gap-3.5 w-full sm:max-w-md m-auto">
                    <li v-for="item in titleSections.limitations.prompts"
                      class="w-full bg-gray-50 dark:bg-white/5 p-3 rounded-md">
                      {{ item }}
                    </li>
                  </ul>
          ...
          <button
            ...
            <svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round"
              stroke-linejoin="round" class="h-4 w-4 m-1" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
              <line x1="12" y1="5" x2="12" y2="19"></line>
              <polyline points="19 12 12 19 5 12"></polyline>
            </svg>
          </button>
        </div>
        <button class="react-scroll-to-bottom--css-wsxdy-1tj0vk3 scroll-convo" type="button"></button>
      </div>
    </div>
    <div
      ...
      <form @submit.prevent="sendRequest"
        ...
        <div class="relative flex h-full flex-1 md:flex-col">
          <div class="hidden md:flex ml-1 md:w-full md:m-auto md:mb-2 gap-0 md:gap-2 justify-center">
          ...
          <div
            ...
            <textarea v-model="prompt" tabindex="0"
              oninput="this.style.height = ''; this.style.height = this.scrollHeight +'px'"
              style="max-height: 200px; height: 24px; overflow-y: hidden" rows="1" placeholder="Send a message..."
              class="m-0 w-full resize-none border-0 bg-transparent p-0 pr-7 focus:ring-0 focus-visible:ring-0 dark:bg-transparent pl-2 md:pl-0"></textarea><button
          ...
          <div class="flex ml-1 md:hidden md:w-full md:m-auto md:mb-2 gap-0 md:gap-2 justify-center">
            <button class="btn relative btn-neutral border-0 md:border">
              <div class="flex w-full items-center justify-center gap-2">
                <svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round"
      ...
      <div class="px-3 pt-2 pb-3 text-center text-xs text-black/50 dark:text-white/50 md:px-4 md:pt-3 md:pb-6">
        <span><a href="https://help.openai.com/en/articles/6825453-chatgpt-release-notes" target="_blank" rel="noreferrer"
            class="underline">ChatGPT Mar 23 Version</a>. Free Research Preview. ChatGPT may produce inaccurate
...
import IconRay from '@/components/icons/IconRay.vue'
...
const { conversation, conversations, activeConversationId } = storeToRefs(conversationStore)
const { clearConversations } = conversationStore

const promptIsEmpty = computed(() => !prompt.value)

function isServerMessage(source: string) {
  return source == 'server'
}

function isClientMessage(source: string) {
...
function sendRequest() {
  ...
  conversation.value.push({ source: 'client', content: prompt.value })
  axios
    .post(
      '/',
      {
